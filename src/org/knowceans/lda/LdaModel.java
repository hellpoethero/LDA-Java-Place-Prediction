...
public class LdaModel {
    ...
    public void saveWordTopicCount(String modelRoot) {
        int i, j;

        String filename = modelRoot + ".wordtopic";

        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(filename));
            for (j = 0; j < this.numTerms; j++) {
                for (i = 0; i < this.numTopics; i++) {
                    bw.write(this.classWord[i][j]/ this.classTotal[i] + " ");
                }
                bw.newLine();
            }
            bw.newLine();
    ...
    public void readVocab(String vocabFile) {
        try {
            ...
            vocab = new String[tempArray.size()];
            for (int wordIndex = 0; wordIndex<tempArray.size(); wordIndex++) {
                vocab[wordIndex] = tempArray.get(wordIndex);
            }
...
//        for (int i=0; i<20 && i<numTerms; i++) {
//            System.out.println(vocab[i]);
//        }

        IDSorter[] sortedWords = new IDSorter[numTerms];
        for (int term = 0; term < numTerms; term++) {
            sortedWords[term] = new IDSorter(term, 0);
        }
        for (int topic = 0; topic < numTopics; topic++) {
            for (int term = 0; term < numTerms; term++) {
//                sortedWords[term] = new IDSorter(term, classWord[topic][term]);
//                System.out.println(term + " " + classWord[topic][term]);
                sortedWords[term].set(term, classWord[topic][term]);
            }
//            for (int i=0; i<20 && i<numTerms; i++) {
////                System.out.println(sortedWords[i].getID() + " " + sortedWords[i].getWeight());
//                System.out.println(vocab[sortedWords[i].getID()] + " " + sortedWords[i].getWeight());
//            }
